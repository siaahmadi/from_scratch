...
`np.stack`  simply creates a new dimension (given by axis)
            and makes the result accessible as though it
            were a Python list. (“add an axis, then concatenate”)
            
            Does not allow different shapes.

`np.concat` allows different shapes along concatenation axis
            sums the sizes along concatenation axis
